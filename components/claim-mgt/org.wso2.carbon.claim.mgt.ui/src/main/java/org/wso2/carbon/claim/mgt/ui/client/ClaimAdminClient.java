...
public class ClaimAdminClient {
    ...
    public ClaimDialectDTO[] getAllClaimMappings() throws AxisFault {
        ...
        try {
            ClaimDialectDTO[] result = stub.getClaimMappings();
            if (log.isDebugEnabled()) {
                log.debug("Successfully retrieved " + (result != null ? result.length : 0) + " claim dialects");
            }
            return result;
        } catch (Exception e) {
    ...
    public ClaimDialectDTO getAllClaimMappingsByDialect(String dialect)
        ...
        try {
            ClaimDialectDTO result = stub.getClaimMappingByDialect(dialect);
            if (log.isDebugEnabled()) {
                log.debug("Successfully retrieved claim mappings for dialect: " + dialect);
            }
            return result;
        } catch (Exception e) {
    ...
    public ClaimDialectDTO getAllClaimMappingsByDialectWithRole(String dialect)
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Successfully added role claims for dialect: " + dialect);
            }
            return claims;
        } catch (Exception e) {
            handleException("Error while reading claim mappings by dialect", e);
    ...
    public void removeClaimMapping(String dialectUri, String claimUri)
            throws AxisFault {
        if (log.isDebugEnabled()) {
            log.debug("Removing claim mapping for dialect: " + dialectUri + ", claim: " + claimUri);
        }
        try {
            stub.removeClaimMapping(dialectUri, claimUri);
            log.info("Successfully removed claim mapping for dialect: " + dialectUri + ", claim: " + claimUri);
        } catch (Exception e) {
